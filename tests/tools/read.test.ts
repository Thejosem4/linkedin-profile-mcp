...
describe('Read Tools', () => {
  ...
  describe('get_profile', () => {
    ...
    it('should fetch profile from LinkedIn if not cached', async () => {
      ...
      jest.spyOn(linkedinClient, 'get').mockResolvedValue(mockApiResponse as any);
      ...
      expect((result.data as any).headline).toBe('Software Engineer');
  ...
  describe('get_analytics', () => {
    it('should return analytics from API', async () => {
      ...
      expect(result.success).toBe(true);
      expect((result.data as any).profileViews).toBe(200);
    });
    ...
    it('should return mock analytics on API failure', async () => {
      ...
      expect((result.data as any).profileViews).toBeDefined();
    });
  });
});
